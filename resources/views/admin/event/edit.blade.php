                ...
                >
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div class="sm:col-span-2">
                                ...
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $event->title)" required autofocus placeholder="Contoh: Klinik Halal UMKM PWM DIY 2025" />
                            ...
                            <div>
                                ...
                                <select id="type" name="type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand text-sm h-[42px]">
                                    ...
                                    <option value="pendampingan" {{ old('type', $event->type) === 'pendampingan' ? 'selected' : '' }}>🤝 Pendampingan</option>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="date_start" name="date_start" type="datetime-local" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand text-sm" value="{{ old('date_start', $event->date_start ? $event->date_start->format('Y-m-d\TH:i') : '') }}" required />
                            ...
                            <div>
                                ...
                                <input id="date_end" name="date_end" type="datetime-local" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand text-sm" value="{{ old('date_end', $event->date_end ? $event->date_end->format('Y-m-d\TH:i') : '') }}" />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="location" name="location" type="text" class="mt-1 block w-full" :value="old('location', $event->location)" required placeholder="Contoh: Aula Gedung PWM D.I. Yogyakarta, Jl. Gedongkuning No. 130 B" />
                                <x-input-error class="mt-2" :messages="$errors->get('location')" />
                            </div>

                            <div>
                                <x-input-label for="contact_person" value="Contact Person (Narahubung)" />
                                <x-text-input id="contact_person" name="contact_person" type="text" class="mt-1 block w-full" :value="old('contact_person', $event->contact_person ?: $event->whatsapp_contact)" placeholder="Contoh: Ibu Amalya | 082280126691" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="10" required placeholder="Tulis deskripsi lengkap kegiatan..." class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-brand focus:ring-brand text-sm font-sans leading-relaxed">{{ old('description', $event->description) }}</textarea>
                        ...
                        <div class="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
                            <div>
                                @if($event->image_path)
                                    <div class="mb-3">
                                        <p class="text-xs font-semibold text-gray-600 mb-1">Banner Saat Ini:</p>
                                        <img src="{{ asset('storage/' . $event->image_path) }}" alt="{{ $event->title }}" class="w-full max-w-sm h-auto rounded-lg shadow-sm border border-gray-200">
                                    </div>
                                @endif

                                <x-input-label for="image" value="Ganti Banner / Poster (Opsional, Format JPG/PNG, Maks. 2MB)" />
                                <input
                                    id="image"
                                    name="image"
                                    type="file"
                                ...
                                <div x-show="previewUrl" class="mt-3" style="display: none;">
                                    <p class="text-xs font-semibold text-gray-600 mb-1">Pratinjau Banner Baru Pengganti:</p>
                                    <img :src="previewUrl" alt="Pratinjau Banner Baru" class="w-full max-w-md h-auto rounded-lg shadow border border-gray-200">
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 pt-1">
                                ...
                                <label for="is_published" class="text-sm font-medium text-gray-700 cursor-pointer">
                                    Publikasikan ke katalog agenda publik
                                </label>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        >
                            ...
                            <span x-text="submitting ? 'Menyimpan...' : 'Perbarui Agenda'">Perbarui Agenda</span>
                        </button>
                    </div>
                </form>
            </div>
